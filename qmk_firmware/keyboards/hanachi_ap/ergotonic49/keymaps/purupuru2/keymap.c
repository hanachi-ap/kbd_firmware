...
[_QWERTY]=LAYOUT(   //QWERTY
	...
	RE_TOGGLE	,							KC_LGUI	,	KC_LALT	,	LT(_NUM,KC_SPC)	,	CTL_T(KC_F13)	,	MO(_NUM)	,	SFT_T(KC_BSPC)	,	LT(_FN, KC_ENT )	,	LT(_NUM, KC_DOWN)	,	LT(_SYS, KC_UP)	,
...
bool process_record_user(uint16_t keycode, keyrecord_t *record)
{
    ...
    case KC_LANG8:
        ...
        break;
    case LT(_FN, KC_ENT):
        if (record->event.pressed && lang_mode)
        {
                // IME ON状態ならモードを直接入力に
